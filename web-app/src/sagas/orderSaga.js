...
const getProductsOfFavourite = async(favouriteId) => {
    const response = await fetch(`${SERWER_PROD}/api/product/products/order/${favouriteId}`,{
        method: 'GET',
        headers: {
            'Accept': 'application/json'
        }
...
const getUserFavouritesProducts = async(userid) => {
    try {
        console.log('getting favourite user');
        let responseData;
        const response = await fetch(`${SERWER_PROD}/api/order/orders/${userid}`,{
            method: 'GET',
            headers: {
                'Accept': 'application/json'
            }
...
const deleteProductFromFavourite = (favourite, product) => {
    try {
        console.log('getting filtered products');
        const response = fetch(`${SERWER_PROD}/api/order/delete/${favourite}/${product}`,{
            method: 'DELETE',
            headers: {
                'Accept': 'application/json',
                'Content-Type': 'application/json'
...
function* loadFavourites (action) {
    try {
        ...
        yield put({ type: 'GET_ORDERS_SUCCESS', payload: favouriteList});
    } catch (e) {
...
function* watchGetFavourites() {
    yield takeLatest('GET_ORDERS', loadFavourites);
}

function* watchAddFavourite() {
    yield takeLatest('ADD_ORDER', addFavouriteFunction);
}

function* watchAddFavouriteProduct() {
    yield takeLatest('ADD_ORDER_PRODUCT', addProductToFavouriteFunction);
}

function* watchDeleteFavouriteProduct() {
    yield takeLatest('DELETE_ORDER_PRODUCT', deleteProductFromFavouriteFunction);
}
